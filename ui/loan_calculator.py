...
# Page configuration
st.set_page_config(
    page_title="RISE Loan Calculator",
    page_icon="💰",
    layout="wide"
)
    ...
    if st.button("Assess Financing Needs", type="primary"):
        with st.spinner("Assessing financing needs..."):
            ...
            if result['success']:
                ...
                with col3:
                    capacity_level = result['repayment_capacity']['capacity_level']
                    st.metric("Repayment Capacity", capacity_level.upper())
    ...
    if st.button("Generate Repayment Schedule", type="primary"):
        with st.spinner("Generating repayment schedule..."):
            ...
            if result['success']:
                ...
                # Color code by payment feasibility
                def highlight_feasibility(row):
                    if row['payment_feasibility'] == 'low':
                        return ['background-color: #ffcccc'] * len(row)
                    elif row['payment_feasibility'] == 'medium':
...
with tab4:
    ...
    if st.button("Generate Document Checklist", type="primary"):
        with st.spinner("Generating document checklist..."):
            farmer_profile = {
                'name': doc_farmer_name,
                'annual_farm_income': 250000
            }
            
            farm_details = {
                'farm_size_acres': doc_farm_size
            }
...
with st.sidebar:
    st.header("ℹ️ About Loan Calculator")
    
    st.markdown("""
    ### Features
    - **Financing Needs Assessment**: Evaluate loan requirements and eligibility
    - **Loan Product Recommendations**: Compare products from banks and NBFCs
    - **Repayment Schedule**: Generate crop-cycle aligned payment plans
    - **Document Helper**: Checklist for loan application
    
    ### Loan Types
    - **Crop Loans**: Short-term for cultivation
    - **Equipment Loans**: Medium-term for machinery
    - **Term Loans**: Long-term for infrastructure
    
    ### Interest Rates
    - Banks: 7-10% p.a.
    - NBFCs: 10-13% p.a.
    - Government schemes: 6-8% p.a.
    
    ### Tips
    - Maintain good credit score (>700)
    - Keep all documents ready
    - Compare multiple lenders
    - Consider interest subvention schemes
    - Align repayment with harvest cycles
    """)
    
    st.info("💡 **Tip:** Apply for Kisan Credit Card for easy access to crop loans with subsidized interest rates!")
