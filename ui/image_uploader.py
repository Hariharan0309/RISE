        ...
        with st.expander(
            ...
            with col3:
                if st.button("🖼️ View Image", key=f"image_{diagnosis.get('diagnosis_id')}", use_container_width=True):
                    s3_key = diagnosis.get('image_s3_key')
                    if s3_key:
                        try:
                            # Generate presigned URL
                            presigned_url = self.disease_tools.generate_presigned_url(s3_key)
                            
                            if presigned_url:
                                st.image(presigned_url, caption=f"Diagnosis Image - {diagnosis.get('diagnosis_id')}", use_container_width=True)
                            else:
                                st.error("❌ Could not generate image URL")
                        except Exception as e:
                            st.error(f"❌ Error loading image: {e}")
                    else:
                        st.warning("⚠️ No image available for this diagnosis")
